...
public class forma3 extends formas {
	private Coordenada x1, x2;
	
	
	public forma3(Coordenada i, int mat[][]) {
		this.inicial = i;
		if( (!valido(i.getX() + 1, i.getY()) || (!valido(i.getX() +1, i.getY() + 1)))){
			c = false;
		}
		else if((mat[i.getX() + 1][i.getY()] == 0) || (mat[i.getX() + 1][i.getY() + 1] == 0)) {
			c = false;
		}
		else {
			 x1 = new Coordenada(i.getX() + 1, i.getY());
